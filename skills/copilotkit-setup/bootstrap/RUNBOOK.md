...
**This runbook does not depend on cloning a git repo.** Bootstrap scaffolds the project from the locally-installed `copilotkit-setup` skill at `~/.claude/skills/copilotkit-setup/`. If the skill isn't installed yet, see the appendix at the bottom of this file.
...
> - Commands are PowerShell (Windows). For macOS/Linux, swap `\Scripts\Activate.ps1` → `bin/activate` and `pwsh` for `bash`.
> - "On failure → 09-debugging-runbook.md#X" refers to [`skills/copilotkit-setup/09-debugging-runbook.md`](skills/copilotkit-setup/09-debugging-runbook.md) (or `~/.claude/skills/copilotkit-setup/09-debugging-runbook.md` after install).
...
**Goal.** Create a fresh project directory with the complete layout. **No GitHub access required** — everything comes from the skill bundled on your machine.

**Commands (PowerShell):**
```powershell
# Pick a target directory. The bootstrap will create it.
$target = "C:\Users\camer\Projects\my-new-app"

# Run the skill's bootstrap.
...
- `-NoGit` / `--no-git`: skip `git init` + initial commit.

**On failure.**
- *"Bootstrap source not found"* → the skill isn't installed at `~/.claude/skills/copilotkit-setup/`. See the **Appendix: Installing the skill** at the bottom of this file.
...
**Commands.**
```powershell
Get-ChildItem -Recurse -File | Measure-Object        # ~75 files
Test-Path .env, backend, frontend, docs, RUNBOOK.md  # all True
Get-Content .env | Select-String "LLM_PROVIDER"      # LLM_PROVIDER=mock
```
**Expected.** ~75 source files. Defaults to `LLM_PROVIDER=mock` (page loads, chat returns `[mock] <message>` until you wire a real key — Step 13).
...
```powershell
# 1. Scaffold from the locally-installed skill (no GitHub access required)
...
Step 1 of this runbook expects the skill at `~/.claude/skills/copilotkit-setup/`. If you don't have it yet:

### Option A — Copy from an existing project that has it

If any other project on this machine has `skills/copilotkit-setup/`:

```powershell
# Windows
$src = "C:\path\to\existing-project\skills\copilotkit-setup"
...
cp -r /path/to/existing-project/skills/copilotkit-setup ~/.claude/skills/copilotkit-setup
```

### Option B — Bootstrap directly from the project's `skills/` directory

If you have the kickstarter project itself anywhere on disk, skip the install and run the bootstrap script straight from there:

```powershell
pwsh "C:\path\to\kickstarter\skills\copilotkit-setup\bootstrap.ps1" -Target C:\path\to\new-project
```

### Option C — Re-create the skill from scratch (last resort)

The skill is just Markdown + the `bootstrap/` directory of source files. Anyone with access to a working CopilotKit Kickstarter project can re-create it by running the same `cp` from there:

```powershell
# From inside the kickstarter project root:
$skill = "$HOME\.claude\skills\copilotkit-setup"
New-Item -ItemType Directory -Force -Path $skill | Out-Null
Copy-Item -Recurse -Force .\skills\copilotkit-setup\* $skill\
```

### Verify the install

```powershell
Test-Path "$HOME\.claude\skills\copilotkit-setup\SKILL.md"          # True
Test-Path "$HOME\.claude\skills\copilotkit-setup\bootstrap\README.md"  # True
Test-Path "$HOME\.claude\skills\copilotkit-setup\bootstrap.ps1"        # True
```

All three must be True. Once installed, **return to Step 1 of this runbook** — bootstrap a fresh project from it.
